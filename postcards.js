/* Registan postcard MVP. Incumbent pixel atlas, paper correspondence and blue tile.
...
(() => {
  'use strict';
  const KEY = 'silk-road.registan-postcards.v1';
  const art = 'assets/registan-postcard-blue.webp';
  const tasks = [
  ...
  const designs = [
    {title:'蓝色来信',subtitle:'骆驼说，它很上镜。',color:'#126571',image:art,blessing:'愿你的日常，也有一小片值得抬头的蓝。'},
  ...
  let state = {started:false,found:[],design:0,to:'亲爱的朋友',from:'一位丝路旅人',message:designs[0].blessing};
  ...
  try {
    ...
    if(saved && typeof saved==='object') {
      ...
      state.found = [...new Set(Array.isArray(saved.found)?saved.found:[])].filter(id=>tasks.some(t=>t.id===id));
      state.design = [0,1,2].includes(saved.design)?saved.design:0;
      for(const field of ['to','from','message']) if(typeof saved[field]==='string') state[field]=saved[field].slice(0,field==='message'?100:24);
    }
  ...
  if(location.hostname==='127.0.0.1'&&location.hash.startsWith('#registan-resume=')){
    try{
      const resume=JSON.parse(decodeURIComponent(location.hash.slice('#registan-resume='.length)));
    ...
    history.replaceState(null,'',location.pathname+location.search+'#registan-quest');
  }
  function save(){try{localStorage.setItem(KEY,JSON.stringify(state));}catch{storageOK=false;} updateScene(currentScene);if(account){clearTimeout(syncTimer);syncTimer=setTimeout(sync,600);}}
  ...
  entry.className='quest-entry'; entry.type='button'; entry.addEventListener('click',()=>open());
  ...
  function updateScene(name){
    entry.textContent=`雷吉斯坦寻宝 · ${state.found.length}/3`;
    sceneEntry.textContent=unlocked()?'我的明信片':'寻宝拼图 · '+state.found.length+'/3';
    sceneEntry.hidden=name!=='plaza';
  ...
  function open(editor=false){
    ...
    dialog.innerHTML=`<!-- THESIS: An unfolding pixel postcard, not a dashboard. OWN-WORLD: lapis, apricot, cream postal paper and stepped edges. STORY: follow three clues, restore a smiling souvenir, write to a friend. --><header class="postcard-header"><div><h2 id="postcard-heading">${editing?'把这一刻，寄给你。':'寻找城市碎片，获取独家记忆'}</h2><p>${editing?'选一款，写祝福。也可以加入自己的照片。':'跟着线索，回广场找找看。'}</p></div>${editing?`<div class="postal-stamp" aria-hidden="true">${mark(2)}<small>SAMARKAND</small></div>`:''}<button type="button" class="postcard-close" aria-label="关闭明信片窗口">×</button></header><div class="postcard-body"></div><p id="postcard-status" role="status" aria-live="polite"></p>`;
  ...
  function renderQuest(){
    ...
    accountButton.onclick=async()=>{
      if(account){
        ...
        try{const response=await fetch('/api/logout',{method:'POST'});if(!response.ok)throw Error();account=null;localStorage.removeItem(KEY);location.reload();}
  ...
  function renderEditor(){
    ...
    dialog.querySelector('#postcard-canvas').setAttribute('aria-label','雷吉斯坦大图背景上的双面明信片：上方祝福、下方合照');
    dialog.querySelector('.postcard-designs').insertAdjacentHTML('afterend',`<section class="selfie-section"><div class="photo-mode" role="group" aria-label="照片选择"><button type="button" data-photo-mode="original" aria-pressed="${photoMode==='original'}">使用原版</button><button type="button" data-photo-mode="personal" aria-pressed="${photoMode==='personal'}">加入我的照片</button></div>${filePreview?'<p class="file-preview-note">直接打开文件时，浏览器会限制照片处理和下载。请从本地网页继续。</p><a id="open-local-postcards" class="local-preview-link" href="http://127.0.0.1:4173/#registan-quest">打开可上传与下载的版本 →</a>':photoMode==='original'?'<p class="selfie-privacy">无需上传照片，写下祝福即可保存或分享。</p>':`<div class="selfie-upload-wrap"><button type="button" class="selfie-upload" id="upload-selfie" ${portraits.length===3||processing?'disabled':''}>${processing?'正在制作像素合影…':portraits.length===3?'已加入 3 人':`上传自拍 · ${portraits.length}/3`} <span aria-hidden="true">＋</span></button><input id="selfie-file" type="file" multiple accept="image/*" ${portraits.length===3||processing?'disabled':''} aria-label="选择最多三张自拍"></div><p class="selfie-privacy">每人一张 · 最多 3 人 · 免费，照片仅在本机处理。</p>${portraits.length?`<div class="selfie-list">${portraits.map((p,i)=>`<span><img src="${p.url}" alt="已加入的第 ${i+1} 位旅人"><button type="button" data-remove-selfie="${i}" ${processing?'disabled':''} aria-label="移除第 ${i+1} 位旅人">×</button></span>`).join('')}</div>`:''}<div class="fusion-progress" ${processing?'':'hidden'} aria-hidden="true"><i></i><span>正在处理，完成后合影会自动刷新。</span></div><p id="selfie-status" role="status" aria-live="polite">${escape(photoStatus)}</p>`}</section>`);
    ...
    if(localLink)localLink.href='http://127.0.0.1:4173/?v=portrait-perspective-6#registan-resume='+encodeURIComponent(JSON.stringify(state));
  ...
  const images=designs.map(d=>{const img=new Image();img.src=d.image;return img;});
  const scenicBackdrop=new Image();scenicBackdrop.src='assets/samarkand-pixel-scene.webp';
  ...
  function correspondence(c,d){
    ...
    c.fillStyle=ink;c.font='700 23px serif';c.fillText('撒马尔罕 · 雷吉斯坦',118,454);
    c.font='17px monospace';c.fillText('SAMARKAND / REGISTAN',118,491);
  ...
  async function paint(){
    ...
    try{
      ...
      // Keep the three original compositions intact on the front of the postcard.
      c.drawImage(image,82,676,916,610.67);
      ...
      preparedFile=new File([blob],`Samarkand-Registan-${state.design+1}.png`,{type:'image/png'});
  ...
  async function share(){
    if(!unlocked()||!preparedFile)return;
    const data={title:'我的雷吉斯坦明信片',text:`${state.to}：${state.message}`,files:[preparedFile]};
  ...
  if(location.hash==='#registan-quest')open();
